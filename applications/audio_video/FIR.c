...
int main()
{
	static int message[COMPRESSED_SAMPLES * 2];
	static int out[COMPRESSED_SAMPLES * 2];

	puts("FIR - start");

	for(int k = 0; k < FRAMES; k++){
		memphis_receive(message, sizeof(message), adpcm_dec);
		unsigned then = memphis_get_tick();

		/* Executes the filter over the input stream */
		fir_filter_int((unsigned*)message, (unsigned*)out, COMPRESSED_SAMPLES*2, (unsigned int*) fir_int, 36, 285);

		/* Sends the adpcm uncompressed stream */
		memphis_send(out, sizeof(out), join);
